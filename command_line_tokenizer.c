...
/**
 * strtow_delim - splits a string into words using a specified delimiter
 * @str: the input string
 * @delim: the delimiter string
 *
 * Return: a pointer to an array of strings, or NULL on failure
 */
char **strtow_delim(char *str, char *delim)
{
	...
	for (i = 0; str[i] != '\0'; i++)
		if (!is_char_delimiter(str[i], delim) &&
		    (is_char_delimiter(str[i + 1], delim) || !str[i + 1]))
			num_words++;
	...
	for (i = 0, j = 0; j < num_words; j++)
	{
		while (is_char_delimiter(str[i], delim))
			i++;
		k = 0;
		while (!is_char_delimiter(str[i + k], delim) && str[i + k])
			k++;
 ...
 * @str: The input string.
 * @delim: The delimiter character used to separate words.
 *
 * Return: A pointer to an array of strings, or NULL on failure.
 */
